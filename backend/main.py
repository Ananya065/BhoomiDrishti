...
def detect_change(req: DetectRequest, db: Session = Depends(get_db)):
    ...
    model_service = get_model_service(sensor=req.sensor or "sentinel2")
    try:
    ...
    record = ChangeRecord(
        ...
        survey_number=None,
        sensor=prediction.get("sensor", "sentinel2"),
        status="needs_review",
...
# ---------- Dashboard / analytics ----------
@app.get("/api/changes/{change_id}/report/html")
def generate_report_html(change_id: str, db: Session = Depends(get_db)):
    """Auto-generated evidence report rendered as HTML (can be printed to PDF)."""
    ...
    <html>
    <head>
        <title>Compliance Report - {r.case_number}</title>
        <style>
            ...
            .header {{ border-bottom: 2px solid #1f4468; padding-bottom: 10px; margin-bottom: 20px; }}
            .header h1 {{ margin: 0; color: #1f4468; }}
            .header p {{ margin: 5px 0 0 0; color: #777; }}
            .section {{ margin-bottom: 30px; }}
            .section h2 {{ border-bottom: 1px solid #ccc; padding-bottom: 5px; color: #1f4468; }}
            ...
            .disclaimer {{ background: #f9f9f9; padding: 15px; border-left: 4px solid #d9822b; font-size: 0.9em; color: #555; }}
        </style>
    </head>
    <body>
        <div class="header">
            <h1>Official Compliance & Detection Report</h1>
            <p>Generated by BhoomiDrishti Automated Intelligence System</p>
        </div>
        
        <div class="section">
            <h2>Case Details</h2>
            <table>
                <tr><th>Case Number</th><td>{r.case_number or 'Pending'}</td></tr>
                <tr><th>Location Name</th><td>{r.location_name}</td></tr>
                <tr><th>Coordinates</th><td>{r.latitude:.5f}, {r.longitude:.5f}</td></tr>
                <tr><th>Detection Date</th><td>{r.detected_at.strftime('%d %b %Y')}</td></tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table>
                <tr><th>Change Type</th><td>{r.change_type}</td></tr>
                <tr><th>Estimated Area</th><td>{r.area_sq_m:.2f} sq meters</td></tr>
                <tr><th>Model Confidence</th><td>{r.confidence * 100:.0f}%</td></tr>
                <tr><th>Severity Score</th><td>{r.severity_score:.1f} / 100</td></tr>
                <tr><th>Sensitive Zone Overlap</th><td>{'Yes' if r.sensitivity_flag else 'No'} {f"({r.sensitivity_zone_name})" if r.sensitivity_zone_name else ''}</td></tr>
            </table>
        </div>
        
        <div class="section">
            <h2>Imagery Evidence</h2>
            <div class="images">
                <div>
                    <strong>Before ({r.before_image_date.strftime('%Y-%m-%d')})</strong><br/>
                    <img src="{r.before_image_url}" alt="Before" />
                </div>
                <div>
                    <strong>After ({r.after_image_date.strftime('%Y-%m-%d')})</strong><br/>
                    <img src="{r.after_image_url}" alt="After" />
                </div>
            </div>
        </div>
        
        <div class="disclaimer">
            <strong>Disclaimer:</strong> This report flags a geographic overlap with a mapped sensitive-zone boundary and/or a detected structural change. It is not a legal determination of ownership, permit status, or violation. All flagged changes require human review against official land records before any enforcement action.
        </div>
