...
def detect_change(req: DetectRequest, db: Session = Depends(get_db)):
    ...
    Currently backed by mock_data.run_mock_detection — swap that one
    function for real Siamese/SNUNet inference + sensitivity overlay
    when the model is ready; this endpoint's shape does not need to change.
    """
    result = mock_data.run_mock_detection(
        req.location_name, req.latitude, req.longitude,
        req.before_image_date, req.after_image_date,
    )
    result.pop("change_type_label", None)
    if req.before_image_url:
        result["before_image_url"] = req.before_image_url
    if req.after_image_url:
        result["after_image_url"] = req.after_image_url

    record = ChangeRecord(**result)
    db.add(record)
    db.commit()
    db.refresh(record)
    return record
